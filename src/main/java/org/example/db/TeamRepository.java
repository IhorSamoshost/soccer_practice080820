...
//    @Query(value = "select * from player_table where player_table.id_team=:idTeam", nativeQuery = true)
    @Query(value = "from Player p where p.team.teamId=:idTeam")
    List<Player> findAllPlayersByTeamId(@Param("idTeam") int teamId);
    ...
    List<Player> findAllPlayersByTeamIdAndPosition(@Param("idTeam") int teamId, @Param("position") String position);
}
